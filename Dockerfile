...
# Copy OpenClaw config bootstrap helpers
COPY bin/lib/openclaw-config.js /opt/nemoclaw-bootstrap/
COPY scripts/render-openclaw-config.js /opt/nemoclaw-bootstrap/
...
# Write the complete openclaw.json from the shared catalog builder.
RUN NEMOCLAW_MODEL="${NEMOCLAW_MODEL}" \
    CHAT_UI_URL="${CHAT_UI_URL}" \
    node /opt/nemoclaw-bootstrap/render-openclaw-config.js /sandbox/.openclaw/openclaw.json

# Install NemoClaw plugin into OpenClaw
RUN openclaw doctor --fix > /dev/null 2>&1 || true \
    && openclaw plugins enable qwen-portal-auth > /dev/null 2>&1 || true \
